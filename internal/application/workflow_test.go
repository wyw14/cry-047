...
func TestRegisterFacilityCreatesAuditTimelineAndNotice(t *testing.T) {
	...
	if facility.State != domain.FacilityNormal || facility.Version != 1 {
		t.Fatalf("unexpected facility: %+v", facility)
	}
	if err := f.store.View(context.Background(), func(tx application.Transaction) error {
		if len(tx.ListTimeline(facility.ID)) != 1 {
			t.Fatalf("timeline count = %d", len(tx.ListTimeline(facility.ID)))
		}
		if len(tx.ListNotifications()) != 1 || len(tx.ListAudit()) != 1 {
			t.Fatalf("side effects missing")
		}
		return nil
	}); err != nil {
		t.Fatal(err)
...
func TestPublishProgramPinsRevisionAndSchedulesNextRun(t *testing.T) {
	...
	if len(program.Revisions) != 1 || program.Revisions[0].Number != 1 {
		t.Fatalf("unexpected revisions: %+v", program.Revisions)
	}
...
func TestReassignWindowUpdatesTaskAndBothNotifications(t *testing.T) {
	...
	if changed.AssigneeID != "person-backup" || changed.Version != window.Version+1 {
		t.Fatalf("unexpected reassignment: %+v", changed)
	}
	if err := f.store.View(context.Background(), func(tx application.Transaction) error {
		if len(tx.ListPersonalTasks("person-backup")) != 1 {
			t.Fatalf("backup task missing")
		}
		if len(tx.ListNotifications()) < 4 {
			t.Fatalf("reassignment notifications missing")
		}
		return nil
	}); err != nil {
		t.Fatal(err)
...
func TestSkipWindowAtomicallyCreatesCompensation(t *testing.T) {
	...
	if skipped.State != domain.WorkSkipped || compensation.CompensatesID != skipped.ID || compensation.State != domain.WorkPlanned {
		t.Fatalf("invalid compensation: %+v %+v", skipped, compensation)
	}
}
...
func TestSubmitExecutionUsesPinnedRevisionAndVerifiesEvidence(t *testing.T) {
	...
	if execution.ProgramRevision != window.ProgramRevision || execution.SubmittedBy != f.maintainer.ID {
		t.Fatalf("unexpected execution: %+v", execution)
	}
	again, err := f.service.SubmitExecution(context.Background(), f.maintainer, submission)
	if err != nil || again.ID != execution.ID {
		t.Fatalf("idempotent retry failed: %+v %v", again, err)
	}
}
...
func TestReviewExecutionTransitionsWindowAndPreservesAudit(t *testing.T) {
	...
	if reviewed.Review == nil || reviewed.Review.Decision != "approve" {
		t.Fatalf("review missing: %+v", reviewed)
	}
	if err := f.store.View(context.Background(), func(tx application.Transaction) error {
		window, err := tx.GetWindow(execution.WindowID)
		if err == nil && window.State != domain.WorkApproved {
			t.Fatalf("window state=%s", window.State)
		}
		return err
	}); err != nil {
		t.Fatal(err)
...
func TestIncidentRecoveryRequiresOrderedReviewWorkflow(t *testing.T) {
	...
	if err != nil || incident.State != domain.IncidentRecovered {
		t.Fatalf("recovery failed: %+v %v", incident, err)
	}
}
...
func TestRiskBoardCombinesOverdueWindowsAndOpenIncidents(t *testing.T) {
	...
	if len(board) != 1 || board[0].FacilityID != window.FacilityID || board[0].OverdueDays == 0 || board[0].OpenIncidents != 1 {
...
func TestExportArchiveIncludesRelatedAggregatesAndChecksum(t *testing.T) {
	...
	if location == "" || len(bundle.Checksum) != 64 || len(bundle.Programs) != 1 || len(bundle.Windows) != 1 || len(bundle.AuditEvents) < 3 {
		t.Fatalf("incomplete archive: %s %+v", location, bundle)
	}
}
